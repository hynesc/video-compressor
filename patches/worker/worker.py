...
def compress_video(self, job_id: str, input_path: str, output_path: str, target_size_mb: float,
    ...
    # --- GEMINI PATCH: FORCE QUALITY MODE (IGNORE TARGET SIZE) ---
    # We ignore the calculated 'video_kbps' and use Constant Quality instead.
    ...
    if actual_encoder.endswith("_nvenc"):
        ...
        # Fallback for others (AMF, VAAPI, etc) - stick to bitrate if unsure, 
        # but let's try to set a high bitrate cap and relax it.
        # For now, we'll just use the original bitrate logic for unsupported hardware 
        # to prevent breaking it, but log a warning.
        quality_flags = ["-b:v", f"{int(video_kbps)}k", "-maxrate", f"{maxrate}k", "-bufsize", f"{bufsize}k"]
